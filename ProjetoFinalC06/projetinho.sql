...
-- Tabela: Desejos (ATUALIZADA - statusDesejo agora é BOOLEAN)
CREATE TABLE IF NOT EXISTS Desejos (
    idDesejos INT PRIMARY KEY NOT NULL AUTO_INCREMENT,
    descricao VARCHAR(50),
    statusDesejo BOOLEAN  -- true = Concedido, false = Não concedido (Negado/Pendente)
);
...
-- Desejos (ATUALIZADO - statusDesejo agora é booleano)
INSERT INTO Desejos (descricao, statusDesejo) VALUES
    ('Quero um lanche infinito', TRUE),       -- Concedido
    ('Quero virar um super-herói', TRUE),    -- Concedido
    ('Quero férias eternas', FALSE),         -- Não concedido
    ('Quero voar', TRUE),                    -- Concedido
    ('Quero um castelo', FALSE),             -- Não concedido
    ('Quero ser o mais inteligente da escola', FALSE), -- Não concedido
    ('Quero um robô ajudante', TRUE),        -- Concedido
    ('Quero entender as meninas', FALSE),    -- Não concedido
    ('Quero ter superpoderes', TRUE),        -- Concedido
    ('Quero um cachorro falante', FALSE);    -- Não concedido
...
UPDATE Desejos SET statusDesejo = TRUE WHERE idDesejos = 5;  -- Castelo agora é concedido
...
SELECT C.nomeCrianca, P.nomePadrinho, D.descricao,
       CASE WHEN D.statusDesejo THEN 'Concedido' ELSE 'Não concedido' END AS status
FROM Padrinhos AS P
...
SELECT C.nomeCrianca, D.descricao, 
       CASE WHEN D.statusDesejo THEN 'Concedido' ELSE 'Não concedido' END AS status
FROM Crianca AS C
JOIN CriancaFazDesejos AS CD ON C.idCrianca = CD.Crianca_idCrianca
JOIN Desejos AS D ON CD.Desejos_idDesejos = D.idDesejos;
